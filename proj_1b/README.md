...
Answers are saved to `results_prompts/<model name>.csv`.
...
Writes `all_models.csv`, one row per prompt per model.
...
prompts/
  starter_prompts.json   the 12 prompts from the assignment
  enriched_prompts.json  prompts we write ourselves
...
purpose. Prompts 7 and 10 need answers from the other prompts first, so they
